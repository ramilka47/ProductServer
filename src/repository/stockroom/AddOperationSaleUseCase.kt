...
class AddOperationSaleUseCase(private val productDao: ProductDao,
    ...
    suspend fun execute(productId : Long, count : Int, price : Double, date : Long) : ProductOperation {
        ...
        if (countEntity.count - count < 0){
            throw RequestException("operation sale is bad. count in stockroom is so few. try less of count")
        }

        return (storageOperationDao.addSale(productId, count, price, date)?.toProductOperation()
